...
# Add rsIDs to LD reference .pvar file
R --vanilla <<EOF
library(tidyverse)
pvar <- read_tsv("${ldref_dir}/ukb_20k_hg19.pvar", col_names=c("CHR", "ID", "POS", "BP", "A1", "A2"), col_types=cols(CHR="c", ID="c"))
...
left_join(pvar, ref, by=c("CHR", "BP")) %>%
  select(CHR, rsID, POS, BP, A1, A2) %>%
  write_tsv("${ldref_dir}/ukb_20k_hg19.pvar", col_names=FALSE)
EOF
